...
- **Everything local.** No model call, memory write, or media generation leaves the machine. This shapes almost every other decision — single-GPU model residency, on-disk memory instead of a hosted database, local TTS instead of a cloud voice API.
- **Skills, not god-object growth.** New capabilities (diagnostics, filesystem access, 3D pipeline work) are built as independent services with their own process, port, and orb panel — not new branches bolted onto the core chat handler. Each skill has a locked scope decided up front, which keeps the core loop from accumulating special cases.
...
**Local models.** Three models held resident simultaneously on one consumer GPU, each with a fixed role rather than one general-purpose model doing everything. Single-GPU residency means VRAM budget is a real constraint — swapping models in and out has a real latency cost, which is a recurring design pressure visible in how the coding sub-mind and the skill routing are structured.

**Graph memory.** Covered in depth in [`MEMORY_SYSTEM.md`](MEMORY_SYSTEM.md) — a typed graph rather than a flat conversation log, with its own compression and protection rules.

**Skill sub-minds.** Independent processes dispatched from the core for a specific job: Raphael watches Cloe's own logs for patterns; Vretil owns the standing filesystem/scheduling relationship (see [`VRETIL.md`](VRETIL.md)). Each ships with its own panel in the orb overlay, matching the overlay's existing visual language rather than introducing a new one per skill.
